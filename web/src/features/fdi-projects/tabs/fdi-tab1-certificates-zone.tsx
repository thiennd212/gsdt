...
import {
  useAddFdiCertificate,
  useUpdateFdiCertificate,
  useDeleteFdiCertificate,
  useFdiProject,
} from '../fdi-project-api';
import type { RegistrationCertificateDto } from '../fdi-project-types';
...
// FdiTab1CertificatesZone — inline CRUD for GCNĐKĐT (registration certificates).
// Add form at top; table below with inline edit (row-level) and delete actions.
export function FdiTab1CertificatesZone({ projectId, disabled }: CertificatesZoneProps) {
  ...
  const addMutation = useAddFdiCertificate();
  const updateMutation = useUpdateFdiCertificate();
  const deleteMutation = useDeleteFdiCertificate();
