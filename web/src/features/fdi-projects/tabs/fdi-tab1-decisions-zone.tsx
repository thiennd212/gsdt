...
import { useAddFdiDecision, useDeleteFdiDecision, useFdiProject } from '../fdi-project-api';
import type { FdiInvestmentDecisionDto } from '../fdi-project-types';
...
// FdiTab1DecisionsZone — two sub-sections: QĐ CTĐT and QĐ ĐT.
// FDI capital split: equityCapital (Vốn CSH) + odaLoanCapital (Vốn ODA) + creditLoanCapital (Vốn TCTD) = totalInvestment.
function FdiDecisionForm({ projectId, decisionType }: { projectId: string; decisionType: number }) {
  const [form] = Form.useForm();
  const addMutation = useAddFdiDecision();
...
export function FdiTab1DecisionsZone({ projectId, disabled }: DecisionsZoneProps) {
  const { data: project } = useFdiProject(projectId);
  const decisions = project?.decisions ?? [];
  const deleteMutation = useDeleteFdiDecision();
  ...
  const columns: ColumnsType<FdiInvestmentDecisionDto> = [
  ...
  return (
    <Space direction="vertical" style={{ width: '100%' }} size={16}>
      ...
      <div>
        ...
        {!disabled && <FdiDecisionForm projectId={projectId} decisionType={DECISION_TYPE_CTDT} />}
        <Table<FdiInvestmentDecisionDto>
      ...
      <div>
        ...
        {!disabled && <FdiDecisionForm projectId={projectId} decisionType={DECISION_TYPE_DT} />}
        <Table<FdiInvestmentDecisionDto>
