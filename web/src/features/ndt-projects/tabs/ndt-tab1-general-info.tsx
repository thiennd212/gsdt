...
import { useCreateNdtProject, useUpdateNdtProject, useNdtProject } from '../ndt-project-api';
import { NdtTab1DecisionsZone } from './ndt-tab1-decisions-zone';
import { NdtTab1LocationsZone } from './ndt-tab1-locations-zone';
import { NdtTab1CertificatesZone } from './ndt-tab1-certificates-zone';
...
// NdtTab1GeneralInfo — orchestrates NĐT general info sections.
// Sections: basic info → decisions (CTĐT/ĐT) → GCNĐKĐT certificates → locations.
// NO DesignEstimate (TKTT) section — NĐT does not have this feature.
export function NdtTab1GeneralInfo({ projectId, mode, onSaved, onDirty }: Tab1Props) {
  const [form] = Form.useForm();
  const navigate = useNavigate();
  const isReadonly = mode === 'detail';

  const { data: project } = useNdtProject(projectId ?? undefined);
  const createMutation = useCreateNdtProject();
  const updateMutation = useUpdateNdtProject();
  ...
  async function handleSave() {
    ...
    if (mode === 'create') {
      createMutation.mutate(values, {
        onSuccess: (data) => {
          message.success('Tạo dự án NĐT thành công');
          onSaved?.();
          navigate({ to: `/ndt-projects/${data.id}/edit` });
        },
    ...
    <div>
      <Form form={form} layout="vertical" disabled={isReadonly} onValuesChange={() => onDirty?.()}>
        ...
        <Card size="small" title="Thông tin cơ bản" style={{ marginBottom: 16 }}>
          ...
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item name="investorName" label="Tên nhà đầu tư trong nước">
      ...
      {projectId && (
        <Card size="small" title="Quyết định CTĐT & ĐT" style={{ marginBottom: 16 }}>
          <NdtTab1DecisionsZone projectId={projectId} disabled={isReadonly} />
        </Card>
      )}
      ...
      {projectId && (
        <Card size="small" title="Giấy chứng nhận đăng ký đầu tư (GCNĐKĐT)" style={{ marginBottom: 16 }}>
          <NdtTab1CertificatesZone projectId={projectId} disabled={isReadonly} />
        </Card>
      )}
      ...
      {projectId && (
        <Card size="small" title="Địa điểm thực hiện" style={{ marginBottom: 16 }}>
          <NdtTab1LocationsZone projectId={projectId} disabled={isReadonly} />
        </Card>
      )}
      ...
      {!isReadonly && (
        <Space style={{ marginTop: 16 }}>
          ...
          <Button onClick={() => navigate({ to: '/ndt-projects' })}>
            Quay lại
          </Button>
        </Space>
      )}
