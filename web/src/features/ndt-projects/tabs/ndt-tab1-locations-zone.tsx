...
import { useAddNdtLocation, useDeleteNdtLocation, useNdtProject, ndtProjectKeys } from '../ndt-project-api';
import type { NdtLocationDto } from '../ndt-project-types';
...
// NdtTab1LocationsZone — inline add/delete with Province → District cascade + KKT/KCN field.
// Extra field `industrialZoneName` captures KKT/KCN/KCX/FTZ/TTTC zone names for NĐT.
export function NdtTab1LocationsZone({ projectId, disabled }: LocationsZoneProps) {
  ...
  const addMutation = useAddNdtLocation();
  const deleteMutation = useDeleteNdtLocation();
  ...
  const columns: ColumnsType<NdtLocationDto> = [
