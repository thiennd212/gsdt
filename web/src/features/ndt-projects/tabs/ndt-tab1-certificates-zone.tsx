...
import {
  useAddNdtCertificate,
  useUpdateNdtCertificate,
  useDeleteNdtCertificate,
  useNdtProject,
} from '../ndt-project-api';
import type { RegistrationCertificateDto } from '../ndt-project-types';
...
// NdtTab1CertificatesZone — inline CRUD for GCNĐKĐT (registration certificates).
// Add form at top; table below with inline edit (row-level) and delete actions.
export function NdtTab1CertificatesZone({ projectId, disabled }: CertificatesZoneProps) {
  ...
  const addMutation = useAddNdtCertificate();
  const updateMutation = useUpdateNdtCertificate();
  const deleteMutation = useDeleteNdtCertificate();
