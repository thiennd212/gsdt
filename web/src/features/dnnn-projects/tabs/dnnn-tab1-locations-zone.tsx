...
import { useAddDnnnLocation, useDeleteDnnnLocation, useDnnnProject, dnnnProjectKeys } from '../dnnn-project-api';
import type { DnnnLocationDto } from '../dnnn-project-types';
...
// DnnnTab1LocationsZone — inline add/delete with Province → District cascade + KKT/KCN field.
// Extra field `industrialZoneName` captures KKT/KCN/KCX/FTZ/TTTC zone names for DNNN.
export function DnnnTab1LocationsZone({ projectId, disabled }: LocationsZoneProps) {
  ...
  const addMutation = useAddDnnnLocation();
  const deleteMutation = useDeleteDnnnLocation();
  ...
  const columns: ColumnsType<DnnnLocationDto> = [
