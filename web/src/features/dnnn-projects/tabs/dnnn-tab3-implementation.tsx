...
import { useAddDnnnBidPackage, useDeleteDnnnBidPackage, useDnnnProject } from '../dnnn-project-api';
import { BidPackageFormModal } from '@/features/domestic-projects/tabs/bid-package-form-modal';
import type { DnnnBidPackageDto } from '../dnnn-project-types';
...
// DnnnTab3Implementation — Tình hình TH for DNNN projects.
// Simpler than PPP Tab3: bid packages section only, no capital plans.
export function DnnnTab3Implementation({ projectId, mode }: Tab3Props) {
  const isReadonly = mode === 'detail';
  const { data: project } = useDnnnProject(projectId);
  const bidPackages = project?.bidPackages ?? [];

  const [bidModalOpen, setBidModalOpen] = useState(false);
  const addBid = useAddDnnnBidPackage();
  const deleteBid = useDeleteDnnnBidPackage();
  ...
  const bidColumns: ColumnsType<DnnnBidPackageDto> = [
