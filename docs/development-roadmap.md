...
| **Phase 32 (P2-P08)** | **v2.46** | **2026-04-09** | **Phase 2 Complete: Build verification, all 6 project types (Domestic/ODA/PPP/DNNN/NĐT/FDI) with ~160 API endpoints + 28 FE components, docs polish, 82 new tests, EF migration AddNdtFdiProjectTypes verified applied (NdtProjects, FdiProjects, NdtInvestmentDecisions, FdiInvestmentDecisions tables confirmed), deferred items noted** | **✓ Complete** |
| **Phase 33** | **v2.47** | **2026-04-10** | **Hardening Sprint: Permission testing infrastructure (TestPermissionSeeder), 32 HTTP integration tests (CQCQ/BTC READ/WRITE/DELETE), 19 contract tests (InvestmentProject+Permission DTOs), 24 security boundary tests (CQCQ read-only, escalation, cross-tenant, delegation, cache), system-architecture.md permission pattern docs. Total: ~75 new tests, bug fix (ReadOnlySetJsonConverter)** | **✓ Complete** |
| **Phase 34** | **v2.48** | **2026-04-10** | **MasterData + SRS UI Hardening: DynamicCatalogsController (tenant-scoped catalog lookups), unified CatalogsController (seed+dynamic merge), seed-masterdata.sql (all 12 tables), Domestic project Tab 1-5 redesign per SRS mockup, inline editable locations table + client-side row management, 34 provinces (QĐ 19/2025 post-merger) + 3321 wards, dual admin model (3-tier+2-tier support), Ward.DistrictCode nullable EF fix, API 'name' field FE compatibility** | **✓ Complete** |
