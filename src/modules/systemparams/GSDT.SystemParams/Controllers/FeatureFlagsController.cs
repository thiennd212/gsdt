...
/// <summary>
/// Admin: manage feature flags (stored as SystemParameter with key prefix "feature:").
    ...
    IFeatureFlagService featureFlagService) : ApiControllerBase
{
        ...
        [FromQuery] string? tenantId, CancellationToken ct)
    {
        ...
        // SetAsync handles DB persist + Redis ff-invalidate publish
        if (tenantId is not null)
            await paramService.SetAsync(key, req.IsEnabled, tenantId, ct);
        else
            await paramService.SetAsync(key, req.IsEnabled, ct);

        return Ok(ApiResponse<object>.Ok(new { key, isEnabled = req.IsEnabled }));
    }

    // --- Public endpoint (no auth — apps check feature availability) ---
