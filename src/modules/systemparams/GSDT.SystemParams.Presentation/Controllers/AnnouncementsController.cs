...
/// Public: GET /api/v1/announcements/active — cached 1min.
/// </summary>
[ApiController]
    ...
    IValidator<CreateAnnouncementRequest> createValidator) : ApiControllerBase
{
    ...
    public async Task<IActionResult> Create([FromBody] CreateAnnouncementRequest req, CancellationToken ct)
    {
        ...
        var announcement = SystemAnnouncement.Create(req.Title, req.Content, req.Severity, req.StartAt, req.EndAt, req.TargetRoles);
    ...
    [Authorize(Roles = "Admin")]
    [EnableRateLimiting("write-ops")]
    public async Task<IActionResult> Update(Guid id, [FromBody] UpdateAnnouncementRequest req, CancellationToken ct)
    {
        var announcement = await db.SystemAnnouncements.FindAsync([id], ct);
        if (announcement is null)
            return NotFound(ApiResponse<object>.Fail([new FluentResults.Error($"Announcement {id} not found.")]));

        announcement.Update(req.Title, req.Content, req.Severity, req.StartAt, req.EndAt, req.IsActive);
        await db.SaveChangesAsync(ct);
        await cache.RemoveAsync(ActiveCacheKey, ct);
    ...
    public async Task<IActionResult> GetActive(CancellationToken ct)
    {
        ...
        if (cached is not null) return Ok(ApiResponse<object>.Ok(cached));
...
// --- Request DTOs + Validator ---

public record CreateAnnouncementRequest(
    string Title, string Content, AnnouncementSeverity Severity,
    DateTimeOffset? StartAt, DateTimeOffset? EndAt, string? TargetRoles);
