...
/// <summary>
/// Foreign direct investment (FDI) project (TPT child table: investment.FdiProjects).
/// Clone of NĐT — identical fields and capital structure (CSH/ODA/TCTD).
/// </summary>
public sealed class FdiProject : InvestmentProject
{
    ...
    // Preliminary capital estimates — same structure: CSH + ODA + TCTD
    ...
    // Navigation properties — FDI children (NO DesignEstimate, NO InvestorSelection)
    public ICollection<FdiInvestmentDecision> InvestmentDecisions { get; set; } = new List<FdiInvestmentDecision>();
    // RegistrationCertificates nav is inherited from InvestmentProject base

    private FdiProject() { } // EF Core

    /// <summary>Factory method — raises ProjectCreatedEvent.</summary>
    public static FdiProject Create(
        Guid tenantId,
        string projectCode,
        string projectName,
    ...
    {
        var project = new FdiProject
        {
            Id = Guid.NewGuid(),
            TenantId = tenantId,
            ProjectCode = projectCode,
            ProjectName = projectName,
            ProjectType = ProjectType.Fdi,
