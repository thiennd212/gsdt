    ...
    : IInvestmentProjectRepository
{
    ...
    public async Task<NdtProject?> GetNdtByIdWithDetailsAsync(
        Guid id, Guid tenantId, CancellationToken ct = default)
        => await context.NdtProjects
            .AsNoTracking()
            .Where(p => p.Id == id && p.TenantId == tenantId)
            .Include(p => p.Locations)
            .Include(p => p.InvestmentDecisions)
    ...
    public async Task<NdtProject?> GetNdtByIdWithDecisionsAsync(Guid id, CancellationToken ct = default)
        => await context.NdtProjects
            .Include(p => p.InvestmentDecisions)
            .FirstOrDefaultAsync(p => p.Id == id, ct);

    public async Task<FdiProject?> GetFdiByIdWithDetailsAsync(
        Guid id, Guid tenantId, CancellationToken ct = default)
        => await context.FdiProjects
            .AsNoTracking()
            .Where(p => p.Id == id && p.TenantId == tenantId)
            .Include(p => p.Locations)
            .Include(p => p.InvestmentDecisions)
    ...
    public async Task<FdiProject?> GetFdiByIdWithDecisionsAsync(Guid id, CancellationToken ct = default)
        => await context.FdiProjects
            .Include(p => p.InvestmentDecisions)
            .FirstOrDefaultAsync(p => p.Id == id, ct);
