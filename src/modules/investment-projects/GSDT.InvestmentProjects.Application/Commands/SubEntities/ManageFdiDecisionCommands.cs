...
/// <summary>Adds an investment decision to an FDI project.</summary>
public sealed record AddFdiDecisionCommand(
    Guid ProjectId,
    int DecisionType,
...
/// <summary>Soft-deletes an investment decision from an FDI project.</summary>
public sealed record DeleteFdiDecisionCommand(
    Guid ProjectId,
    Guid DecisionId)
    : IRequest<Result>;

public sealed class AddFdiDecisionCommandHandler(
    IInvestmentProjectRepository repository,
    ITenantContext tenantContext,
    IProjectQueryScopeService scopeService)
    : IRequestHandler<AddFdiDecisionCommand, Result<Guid>>
{
    public async Task<Result<Guid>> Handle(
        AddFdiDecisionCommand request, CancellationToken cancellationToken)
    {
        ...
        var project = await repository.GetFdiByIdWithDecisionsAsync(request.ProjectId, cancellationToken);
        if (project is null || project.TenantId != tenantId)
            return Result.Fail<Guid>($"GOV_INV_404: Du an FDI khong ton tai (Id={request.ProjectId}).");

        var decision = FdiInvestmentDecision.Create(
...
public sealed class DeleteFdiDecisionCommandHandler(
    IInvestmentProjectRepository repository,
    ITenantContext tenantContext,
    IProjectQueryScopeService scopeService)
    : IRequestHandler<DeleteFdiDecisionCommand, Result>
{
    public async Task<Result> Handle(
        DeleteFdiDecisionCommand request, CancellationToken cancellationToken)
    {
        if (tenantContext.TenantId is not { } tenantId)
        ...
        var project = await repository.GetFdiByIdWithDecisionsAsync(request.ProjectId, cancellationToken);
        if (project is null || project.TenantId != tenantId)
            return Result.Fail($"GOV_INV_404: Du an FDI khong ton tai (Id={request.ProjectId}).");
