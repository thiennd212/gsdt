...
public sealed class UpdatePppProjectCommandValidator
    : AbstractValidator<UpdatePppProjectCommand>
{
    public UpdatePppProjectCommandValidator()
    {
        RuleFor(x => x.Id)
            .NotEmpty().WithMessage("Id du an la bat buoc.");

        RuleFor(x => x.RowVersion)
            .NotNull().WithMessage("RowVersion la bat buoc de kiem tra dong thoi.");
