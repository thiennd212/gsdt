...
/// <summary>
/// Updates fields on an existing FdiProject.
/// Checks optimistic concurrency via RowVersion before writing.
/// </summary>
public sealed class UpdateFdiProjectCommandHandler(
    IInvestmentProjectRepository repository,
    ITenantContext tenantContext,
    IProjectQueryScopeService scopeService)
    : IRequestHandler<UpdateFdiProjectCommand, Result>
{
    public async Task<Result> Handle(
        UpdateFdiProjectCommand request,
        CancellationToken cancellationToken)
    {
        ...
        if (entity is not FdiProject project)
            return Result.Fail("GOV_INV_003: Du an khong phai loai du an FDI.");
