...
namespace GSDT.InvestmentProjects.Application.Commands.UpdateNdtProject;

/// <summary>
/// Updates fields on an existing NdtProject.
/// Checks optimistic concurrency via RowVersion before writing.
/// </summary>
public sealed class UpdateNdtProjectCommandHandler(
    IInvestmentProjectRepository repository,
    ITenantContext tenantContext,
    IProjectQueryScopeService scopeService)
    : IRequestHandler<UpdateNdtProjectCommand, Result>
{
    public async Task<Result> Handle(
        UpdateNdtProjectCommand request,
        CancellationToken cancellationToken)
    {
        ...
        if (entity is not NdtProject project)
            return Result.Fail("GOV_INV_003: Du an khong phai loai du an NDT.");
