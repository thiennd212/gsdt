...
/// <summary>
/// Returns a paginated list of NĐT projects for the current tenant.
/// Uses Dapper + raw SQL. Filterable by search text, competent authority, investor name, status, location.
/// </summary>
public sealed record ListNdtProjectsQuery(
    int Page = 1,
    int PageSize = 50,
    string? Search = null,
    ...
    : IRequest<Result<PagedResult<NdtProjectListItemDto>>>;

public sealed class ListNdtProjectsQueryHandler(
    IReadDbConnection db,
    ITenantContext tenantContext,
    IProjectQueryScopeService scopeService)
    : IRequestHandler<ListNdtProjectsQuery, Result<PagedResult<NdtProjectListItemDto>>>
{
    public async Task<Result<PagedResult<NdtProjectListItemDto>>> Handle(
        ListNdtProjectsQuery request, CancellationToken cancellationToken)
    {
        if (tenantContext.TenantId is not { } tenantId)
            return Result.Fail<PagedResult<NdtProjectListItemDto>>(
        ...
        var countSql = $"""
            ...
            INNER JOIN investment.NdtProjects np ON np.Id = p.Id
            WHERE p.TenantId = @TenantId
              AND p.IsDeleted = 0
              AND (@Search IS NULL
                   OR p.ProjectCode LIKE @SearchLike
                   OR p.ProjectName LIKE @SearchLike)
              AND (@CompetentAuthorityId IS NULL OR np.CompetentAuthorityId = @CompetentAuthorityId)
              AND (@InvestorName IS NULL OR np.InvestorName LIKE @InvestorNameLike)
              AND (@StatusId IS NULL OR np.StatusId = @StatusId)
        ...
        var rows = await db.QueryAsync<NdtProjectListItemDto>(dataSql, param, cancellationToken);
        ...
        return Result.Ok(new PagedResult<NdtProjectListItemDto>(
