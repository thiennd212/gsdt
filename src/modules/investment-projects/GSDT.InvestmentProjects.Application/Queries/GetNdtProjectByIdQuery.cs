...
/// <summary>Returns full detail of a single NĐT project with all child collections.</summary>
public sealed record GetNdtProjectByIdQuery(Guid Id)
    : IRequest<Result<NdtProjectDetailDto>>;

public sealed class GetNdtProjectByIdQueryHandler(
    IInvestmentProjectRepository repository,
    ITenantContext tenantContext)
    : IRequestHandler<GetNdtProjectByIdQuery, Result<NdtProjectDetailDto>>
{
    public async Task<Result<NdtProjectDetailDto>> Handle(
        GetNdtProjectByIdQuery request, CancellationToken cancellationToken)
    {
        if (tenantContext.TenantId is not { } tenantId)
            return Result.Fail<NdtProjectDetailDto>("GOV_INV_401: Tenant context is required.");

        var project = await repository.GetNdtByIdWithDetailsAsync(
            request.Id, tenantId, cancellationToken);

        if (project is null)
            return Result.Fail<NdtProjectDetailDto>(
                $"GOV_INV_404: Du an NDT khong ton tai (Id={request.Id}).");

        return Result.Ok(MapToDetailDto(project));
    }

    private static NdtProjectDetailDto MapToDetailDto(NdtProject p) =>
        new(
            ...
            // NĐT-specific — NO DesignEstimates, NO InvestorSelection
            p.InvestmentDecisions.Select(d => new NdtInvestmentDecisionDto(
