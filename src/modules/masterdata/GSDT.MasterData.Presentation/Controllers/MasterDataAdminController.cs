...
public class MasterDataAdminController(
    ...
    IMemoryCache memoryCache) : ApiControllerBase
{
    // ── Province ──────────────────────────────────────────────────────────────
        ...
        CancellationToken ct)
    {
        ...
        db.Entry(province).CurrentValues.SetValues(new { req.NameVi, req.NameEn, req.SortOrder });
        await db.SaveChangesAsync(ct);
        InvalidateProvinceCache();
        return NoContent();
    }
        ...
        CancellationToken ct)
    {
        ...
        db.Entry(district).CurrentValues.SetValues(new { req.NameVi, req.NameEn, req.SortOrder, req.ProvinceCode });
        await db.SaveChangesAsync(ct);
        InvalidateDistrictCache();
        return NoContent();
    }
        ...
        CancellationToken ct)
    {
        ...
        db.Entry(ward).CurrentValues.SetValues(new { req.NameVi, req.NameEn, req.SortOrder, req.DistrictCode });
        await db.SaveChangesAsync(ct);
        InvalidateWardCache();
        return NoContent();
    }
