...
/// Provides sample data format — real ĐVHC data is project-specific (teams replace seed entries).
    ...
    ILogger<MasterDataSeeder> logger) : IHostedService
{
    public async Task StartAsync(CancellationToken ct)
    {
        ...
        // Build successor/region maps after seeding
        await adminUnitService.BuildCacheAsync(ct);

        // Load hot data into IMemoryCache NeverRemove
    ...
    private static async Task SeedProvincesAsync(MasterDataDbContext db, CancellationToken ct)
    {
        ...
        // Sample provinces — replace with full GSO/BNV dataset in actual projects
        var provinces = new[]
        {
        ...
        // Seed districts for Hà Nội (sample)
        var districts = new[]
        {
    ...
    private static async Task SeedAdministrativeUnitsAsync(MasterDataDbContext db, CancellationToken ct)
    {
        ...
        // Sample entries showing old→new reform chain (Hà Nội 2025 admin reform example)
        var units = new[]
        {
            ...
            // Example: old Ward merged into new Ward (SuccessorCode points to active)
    ...
    /// <summary>
    /// Load Province/District/Ward into IMemoryCache NeverRemove for sub-1ms reads.
    /// </summary>
        ...
        IServiceProvider sp, MasterDataDbContext db, CancellationToken ct)
    {
        ...
        var opts = new Microsoft.Extensions.Caching.Memory.MemoryCacheEntryOptions
        {
            ...
            Size = 1 // Required when IMemoryCache.SizeLimit is set (Cache:L1:MaxEntries)
        };
